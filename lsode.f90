...
C-----------------------------------------------
      ...
      integer, dimension(2) :: mord
      integer :: mu, mxhnl0, mxstp0
      real(rprec) :: atoli, ayi, big, ewti, h0, hmax, hmx, rh,
     1   rtoli, tcrit, tdist, tnext, tol, tolsf, tp, size, sum0, w0
      logical :: ihit
C-----------------------------------------------
...
c next process and check the optional inputs. --------------------------
         if (iopt /= 1) then
            ...
            h0 = rwork(5)
            if ((tout - t)*h0 < 0.0_dp) go to 614
   50       continue
            hmax = rwork(6)
            if (hmax < 0.0_dp) go to 615
            hmxi = 0.0_dp
            if (hmax > 0.0_dp) hmxi = 1.0_dp/hmax
            hmin = rwork(7)
            if (hmin < 0.0_dp) go to 616
c-----------------------------------------------------------------------
...
c check rtol and atol for legality. ------------------------------------
         ...
         do i = 1, n
            ...
            if (rtoli < 0.0_dp) go to 619
            if (atoli < 0.0_dp) go to 620
         end do
         if (istate /= 1) then
...
c-----------------------------------------------------------------------
  ...
  100    continue
         ...
         if (itask==4 .or. itask==5) then
            tcrit = rwork(1)
            if ((tcrit - tout)*(tout - t) < 0.0_dp) go to 625
            if (h0/=0.0_dp .and. (t+h0-tcrit)*h0>0.0_dp) h0 = tcrit - t
         endif
         jstart = 0
         if (miter > 0) rwork(lwm) = sqrt(uround)
         nhnil = 0
...
c load and invert the ewt array.  (h is temporarily set to 1.0.) -------
         ...
         do i = 1, n
            if (rwork(i+lewt-1) <= 0.0_dp) go to 621
            rwork(i+lewt-1) = 1.0_dp/rwork(i+lewt-1)
         end do
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
         if (h0 == 0.0_dp) then
            ...
            if (tdist < 2.0_dp*uround*w0) go to 622
            tol = rtol(1)
            if (itol > 2) then
            ...
            if (tol <= 0.0_dp) then
               ...
               do i = 1, n
                  if (itol==2 .or. itol==4) atoli = atol(i)
                  ayi = abs(y(i))
                  if (ayi /= 0.0_dp) tol = max(tol,atoli/ayi)
               end do
            endif
            tol = max(tol,100.0_dp*uround)
            tol = min(tol,0.001_dp)
            sum0 = vnorm(n,rwork(lf0),rwork(lewt))
            sum0 = 1.0_dp/(tol*w0*w0) + tol*sum0**2
            h0 = 1.0_dp/sqrt(sum0)
            h0 = min(h0,tdist)
            h0 = sign(h0,tout - t)
...
c-----------------------------------------------------------------------
  ...
  210    continue
         if ((tn - tout)*h < 0.0_dp) go to 250
         call intdy (tout, 0, rwork(lyh), nyh, y, iflag)
         if (iflag /= 0) go to 627
         t = tout
         go to 420
  220    continue
         tp = tn - hu*(1.0_dp + 100.0_dp*uround)
         if ((tp - tout)*h > 0.0_dp) go to 623
         if ((tn - tout)*h < 0.0_dp) go to 250
         go to 400
  230    continue
         tcrit = rwork(1)
         if ((tn - tcrit)*h > 0.0_dp) go to 624
         if ((tcrit - tout)*h < 0.0_dp) go to 625
         if ((tn - tout)*h < 0.0_dp) go to 245
         call intdy (tout, 0, rwork(lyh), nyh, y, iflag)
         if (iflag /= 0) go to 627
         t = tout
         go to 420
  ...
  245    continue
         hmx = abs(tn) + abs(h)
         ihit = abs(tn - tcrit) <= 100.0_dp*uround*hmx
         if (ihit) go to 400
         tnext = tn + h*(1.0_dp + 4.0_dp*uround)
         if ((tnext - tcrit)*h > 0.0_dp) then
            h = (tcrit - tn)*(1.0_dp - 4.0_dp*uround)
...
c-----------------------------------------------------------------------
  ...
  250    continue
         ...
         call ewset (n, itol, rtol, atol, rwork(lyh), rwork(lewt))
         do i = 1, n
            if (rwork(i+lewt-1) <= 0.0_dp) go to 510
            rwork(i+lewt-1) = 1.0_dp/rwork(i+lewt-1)
         end do
  270    continue
         tolsf = uround*vnorm(n,rwork(lyh),rwork(lewt))
         if (tolsf > 1.0_dp) then
            tolsf = tolsf*2.0_dp
            if (nst == 0) go to 626
            go to 520
         endif
         if (tn + h == tn) then
            ...
            if (nhnil <= mxhnil) then
               call xerrwv (
     1            'lsode--  warning..internal t (=r1) and h (=r2) are',
     2            50, 101, 0, 0, 0, 0, 0, 0.0_dp, 0.0_dp)
               call xerrwv (
     1 '      such that in the machine, t + h = t on the next step  ', 
     2            60, 101, 0, 0, 0, 0, 0, 0.0_dp, 0.0_dp)
               call xerrwv (
     1            '      (h = step size). solver will continue anyway',
     2            50, 101, 0, 0, 0, 0, 2, tn, h)
               if (nhnil >= mxhnil) then
                  call xerrwv (
     1   'lsode--  above warning has been issued i1 times.  ', 50, 102, 
     2               0, 0, 0, 0, 0, 0.0_dp, 0.0_dp)
                  call xerrwv (
     1   '      it will not be issued again for this problem', 50, 102, 
     2               0, 1, mxhnil, 0, 0, 0.0_dp, 0.0_dp)
               endif
            endif
         endif
c-----------------------------------------------------------------------
...
c itask = 3.  jump to exit if tout was reached. ------------------------
  330    continue
         if ((tn - tout)*h >= 0.0_dp) go to 400
         go to 250
c itask = 4.  see if tout or tcrit was reached.  adjust h if necessary.
  340    continue
         if ((tn - tout)*h >= 0.0_dp) then
            call intdy (tout, 0, rwork(lyh), nyh, y, iflag)
            t = tout
            go to 420
         endif
         ...
         if (ihit) go to 400
         tnext = tn + h*(1.0_dp + 4.0_dp*uround)
         if ((tnext - tcrit)*h <= 0.0_dp) go to 250
         h = (tcrit - tn)*(1.0_dp - 4.0_dp*uround)
         jstart = -2
         go to 250
...
c the maximum number of steps was taken before reaching tout. ----------
  500    continue
     ...
     2      201, 0, 0, 0, 0, 0, 0.0_dp, 0.0_dp)
         call xerrwv (
     1      '      taken on this call before reaching tout     ', 50, 
     2      201, 0, 1, mxstep, 0, 1, tn, 0.0_dp)
         istate = -1
         go to 580
...
c
  700 continue
     ...
     1   , 50, 302, 0, 0, 0, 0, 0, 0.0_dp, 0.0_dp)
c
  800 continue
      call xerrwv ('lsode--  run aborted.. apparent infinite loop     '
     1   , 50, 303, 2, 0, 0, 0, 0, 0.0_dp, 0.0_dp)
...
!     end subroutine cfode
      ...
      USE kind_spec
C...Translated by Pacific-Sierra Research VAST-90 2.06G2  18:48:15  12/ 3/99  
C...Switches: -p4 -yb
      implicit none
C-----------------------------------------------
C   D u m m y   A r g u m e n t s
...
c-----------------------------------------------------------------------
      ...
      if (k>=0 .and. k<=nq) then
         tp = tn - hu - 100.0_dp*uround*(tn + hu)
         if ((t - tp)*(t - tn) > 0.0_dp) go to 90
c
         s = (t - tn)/h
         ic = 1
         if (k /= 0) then
...
C-----------------------------------------------
     ...
     1   meband, ml, ml3, mu, np1
      real(rprec) :: con, di, fac, hl0, r, r0, srur, yi, yj, yjj
C-----------------------------------------------
C   E x t e r n a l   F u n c t i o n s
C-----------------------------------------------
      real(rprec) , EXTERNAL :: vnorm
...
c do lu decomposition on p. --------------------------------------------
      call gefa_g (wm(3), n, n, iwm(21), ier)
      if (ier /= 0) ierpj = 1
      return 
...
c do lu decomposition of p. --------------------------------------------
      call gbfa_g (wm(3), meband, n, ml, mu, iwm(21), ier)
      if (ier /= 0) ierpj = 1
...
c-----------------------------------------------------------------------
      ...
      select case (miter) 
      case default
         call gesl_g (wm(3), n, n, iwm(21), x, 0)
         return 
c
      case (3) 
         phl0 = wm(2)
...
c
      case (4:5) 
         ...
         mu = iwm(2)
         meband = 2*ml + mu + 1
         call gbsl_g (wm(3), meband, n, ml, mu, iwm(21), x, 0)
         return 
      end select
...
!     end subroutine solsy
      ...
      subroutine stode(neq, y, yh, nyh, yh1, ewt, savf, acor, wm, iwm, f
     1   , jac, pjac, slvs)
      USE kind_spec
C...Translated by Pacific-Sierra Research VAST-90 2.06G2  18:48:15  12/ 3/99  
C...Switches: -p4 -yb
...
c-----------------------------------------------------------------------
  ...
  120    continue
         ...
         exdn = 1.0_dp/(l)
         rhdn = 1.0_dp/(1.3_dp*ddn**exdn + 0.0000013_dp)
         rh = min(rhdn,1.0_dp)
         iredo = 3
         if (h == hold) go to 170
...
c-----------------------------------------------------------------------
  ...
  175 continue
      rh = min(rh,rmax)
      rh = rh/max(1.0_dp,abs(h)*hmxi*rh)
      r = 1.0_dp
      do j = 2, l
         r = r*rh
         yh(:n,j) = yh(:n,j)*r
...
c-----------------------------------------------------------------------
  200 continue
      if (abs(rc - 1.0_dp) > ccmax) ipup = miter
...
c-----------------------------------------------------------------------
      ...
      dcon = del*min(1.0_dp,1.5_dp*crate)/(tesco(2,nq)*conit)
      if (dcon <= 1.0_dp) go to 450
      m = m + 1
      if (m == maxcor) go to 410
      if (m>=2 .and. del>2.0_dp*delp) go to 410
      delp = del
      call f (neq, tn, y, savf)
      nfe = nfe + 1
      go to 270
...
c-----------------------------------------------------------------------
  410 continue
      if (miter==0 .or. jcur==1) go to 430
      icf = 1
      ipup = miter
      go to 220
  430 continue
...
cdir$ ivdep
      ...
      if (abs(h) <= hmin*1.00001_dp) go to 670
...
cdir$ ivdep
      ...
      rmax = 2.0_dp
      if (abs(h) <= hmin*1.00001_dp) go to 660
...
c-----------------------------------------------------------------------
  520 continue
      ...
      if (l /= lmax) then
         ...
         exup = 1.0_dp/(l + 1)
         rhup = 1.0_dp/(1.4_dp*dup**exup + 0.0000014_dp)
      endif
  540 continue
      exsm = 1.0_dp/(l)
      rhsm = 1.0_dp/(1.2_dp*dsm**exsm + 0.0000012_dp)
      rhdn = 0.0_dp
      if (nq /= 1) then
  ...
  580 continue
      newq = nq - 1
      rh = rhdn
      if (kflag<0 .and. rh>1.0_dp) rh = 1.0_dp
      go to 620
  590 continue
      newq = l
      rh = rhup
      if (rh < 1.1_dp) go to 610
      r = el(l)/(l)
  ...
  610 continue
      ...
      go to 700
  620 continue
      if (kflag==0 .and. rh<1.1_dp) go to 610
...
C   L o c a l   V a r i a b l e s
C-----------------------------------------------
      integer :: i, lun, ncpw, nch, nwds
      integer ::  mesflg = 1, lunit = 6
C-----------------------------------------------
C   E x t e r n a l   F u n c t i o n s
C-----------------------------------------------
c     real(rprec) , EXTERNAL :: write
c-----------------------------------------------------------------------
...
c 10  format(1x,30a2)
c-----------------------------------------------------------------------
         if (ni == 1) write (lun, 20) i1
   20    format(6x,'in above message,  i1 =',i10)
         if (ni == 2) write (lun, 30) i1, i2
   30    format(6x,'in above message,  i1 =',i10,3x,'i2 =',i10)
         if (nr == 1) write (lun, 40) r1
   40    format(6x,'in above message,  r1 =',d21.13)
         if (nr == 2) write (lun, 50) r1, r2
