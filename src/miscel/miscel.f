...
c       ierror = putenv(temp)

      ! jons: hack to get linker happy
      print *, "putenv '", temp, "'"
      ierror = 0
...
c       integer, external :: chdir
c       vmec_chdir = chdir(trim(new_path) // char(0))
      ! jons: hack to get linker happy
      print *, "chdir '", trim(new_path), "'"
      vmec_chdir = 0
